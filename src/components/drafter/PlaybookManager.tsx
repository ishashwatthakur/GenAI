...
// Describes the structure of a single Playbook object
interface Playbook {
  id: string;
  name: string;
  description: string;
  rules: Record<string, string>; // An object with string keys and string values
}
...
export default function PlaybookManager({
  activePlaybook,
  onPlaybookChange
}: PlaybookManagerProps): JSX.Element {
  const [isOpen, setIsOpen] = useState<boolean>(false);
  const [showCreateModal, setShowCreateModal] = useState<boolean>(false);

  const currentPlaybook: Playbook = DEFAULT_PLAYBOOKS.find((p: Playbook) => p.id === activePlaybook) || DEFAULT_PLAYBOOKS[0];

  const handlePlaybookSelect = (id: string): void => {
    onPlaybookChange(id);
    setIsOpen(false);
  };
        ...
        <>
          ...
          <div className="absolute right-0 mt-2 w-80 bg-slate-900 border border-white/20 rounded-xl shadow-2xl z-50 overflow-hidden">
            ...
            <div className="max-h-96 overflow-y-auto">
              {DEFAULT_PLAYBOOKS.map((playbook: Playbook) => (
                <button
                  key={playbook.id}
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-1">
                    {Object.keys(playbook.rules).slice(0, 3).map((rule: string) => (
                      <span 
                        key={rule}
