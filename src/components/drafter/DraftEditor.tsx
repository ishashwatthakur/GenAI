...
export default function DraftEditor({
  ...
  const [editedDraft, setEditedDraft] = useState<string>(initialDraft);
  ...
  const handleReview = async (): Promise<void> => {
    ...
    try {
      ...
      if (data.success) {
        ...
        alert(`Review failed: ${data.error || 'Unknown API error'}`);
        setReviewResults(null);
      }
    } catch (error) {
      console.error('Review failed:', error);
      alert('An unexpected error occurred during the review process.');
    } finally {
  ...
  const handleApplyImprovement = (): void => {
          ...
          >
            ...
            {reviewResults.issues && reviewResults.issues.length > 0 && (
              <div className="space-y-3">
                ...
                {reviewResults.issues.map((issue: ReviewIssue, index: number) => (
                  <div
                    key={index}
              ...
              <div className="space-y-2">
                ...
                <div className="bg-white/10 rounded-lg p-4">
                  <ul className="list-disc list-inside space-y-1 text-white/80">
                    {reviewResults.missingClauses.map((clause: string, index: number) => (
