...
// Describes the structure of the data sent to the parent on submit
...
export default function PlainLanguageInput({
  documentType,
  onSubmit,
  isGenerating,
}: PlainLanguageInputProps): JSX.Element {
  const [prompt, setPrompt] = useState<string>('');
  const [jurisdiction, setJurisdiction] = useState<string>('US Federal');
  const [includePlaybook, setIncludePlaybook] = useState<boolean>(true);
  ...
  const handleJurisdictionChange = (e: ChangeEvent<HTMLSelectElement>): void => {
    setJurisdiction(e.target.value);
  };

  const handlePlaybookChange = (e: ChangeEvent<HTMLInputElement>): void => {
    setIncludePlaybook(e.target.checked);
  };

  const handleAdditionalRequirementsChange = (e: ChangeEvent<HTMLTextAreaElement>): void => {
    setAdditionalRequirements(e.target.value);
  };

  const wordCount: number = prompt.trim().split(/\s+/).length;
  const isValidLength: boolean = wordCount >= 10 && wordCount <= 500;
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2 max-h-48 overflow-y-auto">
          {EXAMPLE_PROMPTS.map((example: string, index: number) => (
            <motion.button
              key={index}
              type="button"
