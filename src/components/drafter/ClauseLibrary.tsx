...
interface ClauseLibraryProps {
  onGenerateDraft: (draftText: string) => void;
  isGenerating: boolean;
}

const SAMPLE_CLAUSES: Record<string, Clause[]> = {
  confidentiality: [
    {
      id: 'conf-1',
...
export default function ClauseLibrary({ 
  onGenerateDraft, 
  isGenerating 
}: ClauseLibraryProps): JSX.Element {
  const [selectedClauses, setSelectedClauses] = useState<string[]>([]);
  const [activeCategory, setActiveCategory] = useState<string>('confidentiality');
  const [searchQuery, setSearchQuery] = useState<string>('');

  const categories: string[] = Object.keys(SAMPLE_CLAUSES);
  const currentClauses: Clause[] = SAMPLE_CLAUSES[activeCategory] || [];

  const filteredClauses: Clause[] = currentClauses.filter((clause: Clause) =>
    clause.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
    clause.text.toLowerCase().includes(searchQuery.toLowerCase())
  );

  const toggleClause = (clauseId: string): void => {
    setSelectedClauses((prev: string[]) =>
      prev.includes(clauseId)
        ? prev.filter((id: string) => id !== clauseId)
        : [...prev, clauseId]
    );
  };

  const handleGenerateDraft = (): void => {
    const allClauses: Clause[] = Object.values(SAMPLE_CLAUSES).flat();
    const selected = allClauses.filter((c: Clause) => selectedClauses.includes(c.id));
    const draftText = selected.map(c => c.text).join('\n\n');
