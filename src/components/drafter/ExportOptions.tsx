...
export default function ExportOptions({
  ...
  const handleGeneratePDFPreview = async (): Promise<void> => {
    setIsGenerating(true);
    try {
      const bytes: Uint8Array = await generatePDFWithTemplate(draft, documentTitle, documentType);
      setPdfBytes(bytes);
      
      const blobUrl: string = createPDFBlobUrl(bytes);
  ...
  const handleDownloadPDF = async (): Promise<void> => {
    try {
      let bytes: Uint8Array | null = pdfBytes;
      ...
      const filename: string = `${documentType || 'document'}-${Date.now()}.pdf`;
