...
import type { AnalysisResult, Party } from '../types/analysis';
...
const fadeInUp: Variants = {
  ...
  exit: { opacity: 0, y: -60, transition: { duration: 0.6, ease: [0.6, -0.05, 0.01, 0.99] } }
};
...
interface AnalysisResultsProps {
  ...
  uploadSectionRef: RefObject<HTMLElement>;
}

// Helper function to safely handle the `parties` array
function formatParties(parties: (string | Party)[] | undefined): string {
    if (!parties || parties.length === 0) {
      return 'Not specified';
    }
    // Check if the first item is a string to determine the array type
    if (typeof parties[0] === 'string') {
      return (parties as string[]).join(' & ');
    }
    // Otherwise, it's an array of Party objects
    return (parties as Party[]).map(p => p.name).join(' & ');
}
...
export default function AnalysisResults({
  ...
  onAnalyzeAnother,
  uploadSectionRef
}: AnalysisResultsProps): JSX.Element | null {
  const [showChatbotHint, setShowChatbotHint] = useState<boolean>(true);
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        >
            ...
            <motion.div variants={fadeInUp} className="group relative">
              ...
              <div className="relative bg-gray-900/80 backdrop-blur-xl border border-gray-800/50 rounded-3xl p-8 shadow-2xl hover:shadow-blue-500/10 transition-all duration-500">
                ...
                <div className="space-y-3">
                  {[
                    { icon: '👥', label: 'Parties', value: formatParties(analysis.parties) },
            ...
            <motion.div variants={fadeInUp} className="relative group">
              ...
              <div className="relative bg-gray-900/80 backdrop-blur-xl border border-gray-800/50 rounded-3xl p-8 shadow-2xl hover:shadow-emerald-500/10 transition-all duration-500 h-full">
                ...
                <div className="space-y-4">
                  {[
                    { label: 'Critical Issues', value: criticalCount, icon: '🚨', color: 'text-red-400', bgColor: 'bg-red-500/10', borderColor: 'border-red-500/30' },
                    { label: 'Warnings', value: warningCount, icon: '⚠️', color: 'text-yellow-400', bgColor: 'bg-yellow-500/10', borderColor: 'border-yellow-500/30' },
                    { label: 'Positive Clauses', value: positiveCount, icon: '✅', color: 'text-green-400', bgColor: 'bg-green-500/10', borderColor: 'border-green-500/30' },
                    { label: 'Action Items', value: actionCount, icon: '📋', color: 'text-blue-400', bgColor: 'bg-blue-500/10', borderColor: 'border-blue-500/30' }
                  ].map((metric, index) => (
                    <motion.div
                      key={metric.label}
          ...
          >
            ...
            <motion.button
              whileHover={{ scale: 1.05, boxShadow: "0 20px 60px rgba(75, 85, 99, 0.4)" }}
              whileTap={{ scale: 0.98 }}
