...
// Define the possible priority levels for type safety
type Priority = 'high' | 'medium' | 'low';

// Describes the structure of a single Action Item
...
// Return type for priority configuration
...
export default function ActionItems({
  ...
  const toggleItem = (index: number): void => {
    setExpandedItems((prev: Record<number, boolean>) => ({ ...prev, [index]: !prev[index] }));
  };

  const getPriorityConfig = (priority?: Priority | string): PriorityConfig => {
      ...
      <div className="space-y-4">
        ...
        <div className="grid gap-4">
          {actionItems.map((item: ActionItem, index: number) => {
            const config: PriorityConfig = getPriorityConfig(item.priority);
      ...
      {negotiationPoints.length > 0 && (
        ...
        >
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {negotiationPoints.map((point: string, index: number) => (
              <motion.div
                key={index}
