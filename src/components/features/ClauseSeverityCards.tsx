...
// Use a specific type for severity to prevent typos and ensure consistency
type Severity = 'critical' | 'warning' | 'safe';

// Describes the structure of a single clause object
...
interface ClauseSeverityCardsProps {
  clauses?: FlaggedClause[];
}

// Return type for severity configuration
...
export default function ClauseSeverityCards({
  clauses = []
}: ClauseSeverityCardsProps): JSX.Element {
  const [expandedCards, setExpandedCards] = useState<Record<string | number, boolean>>({});

  const toggleCard = (id: string | number): void => {
    setExpandedCards((prev: Record<string | number, boolean>) => ({ 
      ...prev, 
      [id]: !prev[id] 
    }));
  };

  const getSeverityConfig = (severity: Severity): SeverityConfig => {
    switch (severity) {
      case 'critical': 
        return {
          icon: '🚨',
  ...
  const criticalClauses: FlaggedClause[] = clauses.filter((c: FlaggedClause) => c.severity === 'critical');
  const warningClauses: FlaggedClause[] = clauses.filter((c: FlaggedClause) => c.severity === 'warning');
  const safeClauses: FlaggedClause[] = clauses.filter((c: FlaggedClause) => c.severity === 'safe');

  const renderClauseGroup = (
    clauseGroup: FlaggedClause[], 
    groupTitle: string, 
    groupDelay: number
  ): JSX.Element | null => {
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="grid gap-4">
          {clauseGroup.map((clause: FlaggedClause, index: number) => {
            const config: SeverityConfig = getSeverityConfig(clause.severity);
