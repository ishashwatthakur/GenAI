...
// Severity configuration return type
...
export default function DocumentHeatmap({ 
  clauses = [], 
  totalPages = 10 
}: DocumentHeatmapProps): JSX.Element {
  const [hoveredClause, setHoveredClause] = useState<Clause | null>(null);
  const [view, setView] = useState<'heatmap' | 'list'>('heatmap');
  const [selectedSeverity, setSelectedSeverity] = useState<'all' | 'critical' | 'warning' | 'safe'>('all');

  const getClausePosition = (clause: Clause): number => {
  ...
  const getSeverityConfig = (severity: Clause['severity']): SeverityConfig => {
    switch (severity) {
      case 'critical': 
        return {
          color: 'bg-red-500',
  ...
  const clausesBySeverity: Record<'critical' | 'warning' | 'safe', Clause[]> = {
  ...
  const filteredClauses: Clause[] = selectedSeverity === 'all' 
    ? clauses 
    : clauses.filter(c => c.severity === selectedSeverity);

  // Calculate risk score
  const riskScore: number = clauses.length > 0 
    ? Math.round(
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          {([
            { key: 'all' as const, label: 'All Clauses', icon: '📊', count: clauses.length },
            { key: 'critical' as const, label: 'Critical', icon: '🚨', count: clausesBySeverity.critical.length },
            { key: 'warning' as const, label: 'Warnings', icon: '⚠️', count: clausesBySeverity.warning.length },
            { key: 'safe' as const, label: 'Safe', icon: '✅', count: clausesBySeverity.safe.length }
          ] as FilterOption[]).map((filter: FilterOption) => (
            <motion.button
              key={filter.key}
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
        ...
        {view === 'heatmap' ? (
          ...
          >
              ...
              <div className="bg-gradient-to-br from-gray-900 via-purple-900/20 to-gray-900 rounded-2xl p-6 border-2 border-purple-500/30 shadow-2xl">
                ...
                <div className="bg-gradient-to-br from-gray-800/50 to-gray-900/50 rounded-xl p-4 border border-purple-500/20">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    {([
                    ...
                    ] as SectionZone[]).map((section: SectionZone, idx: number) => {
                      
                      // Calculate clauses in this section
                      const sectionClauses = clauses.filter((c: Clause) => {
                      ...
                      const criticalCount = sectionClauses.filter((c: Clause) => c.severity === 'critical').length;
                      const warningCount = sectionClauses.filter((c: Clause) => c.severity === 'warning').length;
                      const safeCount = sectionClauses.filter((c: Clause) => c.severity === 'safe').length;

                      // Determine heat level
                      const heatLevel: 'high' | 'medium' | 'low' = criticalCount > 0 ? 'high' : warningCount > 1 ? 'medium' : 'low';
                      const heatConfig: Record<'high' | 'medium' | 'low', HeatConfig> = {
                      ...
                      return (
                        <motion.div
                          ...
                          whileHover={{ scale: 1.05 }}
                          className={`bg-gradient-to-br ${heatConfig[heatLevel].bg} rounded-xl p-4 border-2 ${heatConfig[heatLevel].border} cursor-pointer`}
                        >
                          {/* Header */}
                          <div className="flex items-center justify-between mb-3">
                            ...
                            <span className="text-3xl">{heatConfig[heatLevel].icon}</span>
                          </div>
                          ...
                          <div className="space-y-2">
                            ...
                            <div className={`mt-2 p-2 bg-black/30 rounded text-center`}>
                              <p className={`text-xs font-semibold ${heatConfig[heatLevel].text}`}>
                    ...
                    {hoveredClause.suggestions && hoveredClause.suggestions.length > 0 && (
                      <div>
                        ...
                        <ul className="space-y-1">
                          {hoveredClause.suggestions.map((suggestion: string, idx: number) => (
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                { severity: 'critical' as const, count: clausesBySeverity.critical.length },
                { severity: 'warning' as const, count: clausesBySeverity.warning.length },
                { severity: 'safe' as const, count: clausesBySeverity.safe.length },
                { severity: 'total' as const, count: clauses.length }
          ...
          >
            {filteredClauses.length === 0 ? (
              ...
              filteredClauses.map((clause: Clause, index: number) => {
                const config = getSeverityConfig(clause.severity);
