...
// Describes the color configuration for the risk meter
...
export default function RiskMeter({ 
  ...
  const config: ColorConfig = getColorConfig();
  const rotation: number = (animatedScore / 100) * 180 - 90;
    ...
    <div className="relative w-full max-w-sm mx-auto">
      ...
      <div className="relative aspect-[2/1] w-full">
        ...
        <svg className="absolute inset-0 w-full h-full" viewBox="0 0 200 100" preserveAspectRatio="xMidYMid meet">
          ...
          {/* Tick marks */}
          {[0, 25, 50, 75, 100].map((tick: number) => {
            const angle: number = ((tick / 100) * 180 - 90) * (Math.PI / 180);
            const x1: number = 100 + 55 * Math.cos(angle);
            const y1: number = 80 + 55 * Math.sin(angle);
            const x2: number = 100 + 65 * Math.cos(angle);
            const y2: number = 80 + 65 * Math.sin(angle);
      ...
      <div className="absolute top-4 right-4 flex flex-col gap-2">
        {indicators.map((item: IndicatorItem, index: number) => (
          item.value > 0 && (
            <motion.div
              key={item.label}
