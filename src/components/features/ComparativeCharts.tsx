...
// Describes the data coming from your industry comparison API
...
// Describes the metadata about the user's document
interface Metadata {
  pageCount?: number;
}
...
// Describes the structure of a single metric object used for rendering charts
interface Metric {
  label: string;
  value: number;
  comparison: string;
...
export default function ComparativeCharts({
  industryComparison,
  metadata
}: ComparativeChartsProps): JSX.Element {
  ...
  const getComparisonValue = (comparison?: string): number => {
    if (!comparison) return 50;
    const lower: string = comparison.toLowerCase();
  ...
  const getComparisonColor = (comparison?: string): string => {
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400 leading-relaxed">
              Your contract performs {metrics.filter((m: Metric) => m.value > 55).length} out of {metrics.length} metrics above industry average.
              {metrics.filter((m: Metric) => m.value < 45).length > 0 && ` Consider reviewing ${metrics.filter((m: Metric) => m.value < 45).length} area(s) that fall below standard.`}
            </p>
          </div>
        </div>
      </motion.div>
