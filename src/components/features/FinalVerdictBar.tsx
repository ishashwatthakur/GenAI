...
// Individual flagged clause structure
...
// Describes the structure of the final verdict object
...
interface FinalVerdictBarProps {
  finalVerdict: FinalVerdict | null;
  flaggedClauses?: FlaggedClause[];
}

// Segment structure for the bar chart
interface Segment {
  label: string;
  count: number;
  percentage: number;
...
// Recommendation configuration return type
...
export default function FinalVerdictBar({ 
  finalVerdict, 
  flaggedClauses = [] 
}: FinalVerdictBarProps): JSX.Element {
  ...
  const criticalCount: number = flaggedClauses.filter((c: FlaggedClause) => c.severity === 'critical').length;
  const warningCount: number = flaggedClauses.filter((c: FlaggedClause) => c.severity === 'warning').length;
  const safeCount: number = flaggedClauses.filter((c: FlaggedClause) => c.severity === 'safe').length;
  const total: number = criticalCount + warningCount + safeCount || 1;
  ...
  const getRecommendationConfig = (recommendation?: string): RecommendationConfig => {
  ...
  const config: RecommendationConfig = getRecommendationConfig(finalVerdict?.recommendation);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Main Concerns */}
      {/* FIX: Use optional chaining here in case finalVerdict is null */}
      {finalVerdict?.mainConcerns && finalVerdict.mainConcerns.length > 0 && (
        ...
        >
          ...
          <div className="grid md:grid-cols-2 gap-3">
            {finalVerdict.mainConcerns.map((concern: string, index: number) => (
              <motion.div
                key={index}
                initial={{ opacity: 0, x: -20 }}
                animate={{ opacity: 1, x: 0 }}
