...
// Describes the structure of a single positive provision object
interface PositiveProvision {
  title: string;
  benefit: string;
  location: string;
  description?: string;
...
interface PositiveClausesCarouselProps {
  positiveProvisions?: PositiveProvision[];
}

export default function PositiveClausesCarousel({
  positiveProvisions = []
}: PositiveClausesCarouselProps): JSX.Element {
  ...
  useEffect(() => {
    if (positiveProvisions.length > 1 && !isPaused) {
      const interval = setInterval(() => {
        setDirection(1);
        setCurrentIndex((prev: number) => (prev + 1) % positiveProvisions.length);
      }, 5000);
  ...
  const handlePrev = (): void => {
    setDirection(-1);
    setCurrentIndex((prev: number) => (prev - 1 + positiveProvisions.length) % positiveProvisions.length);
  };
      ...
      {positiveProvisions.length > 1 && (
        <>
          ...
          <div className="flex justify-center gap-3 mt-6">
            {positiveProvisions.map((_: PositiveProvision, index: number) => (
              <motion.button
                key={index}
                onClick={() => {
