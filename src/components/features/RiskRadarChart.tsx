...
// Describes the structure of the risk data object passed in as a prop
...
// Describes the props your component receives from its parent
interface RiskRadarChartProps {
  riskCategories?: RiskCategories;
}

// Describes the structure of a single category used for rendering the chart
interface ChartCategory {
  key: keyof RiskCategories;
  label: string;
  value: number;
  icon: string;
}

// Describes a point on the radar chart
...
// Describes color configuration for different risk values
...
export default function RiskRadarChart({ 
  riskCategories = {} 
}: RiskRadarChartProps): JSX.Element {
  ...
  const angleStep: number = (2 * Math.PI) / categories.length;

  const points: RadarPoint[] = categories.map((cat: ChartCategory, index: number) => {
    const angle: number = index * angleStep - Math.PI / 2;
    const value: number = cat.value / 100;
    return {
  ...
  const pathData: string = points.map((point: RadarPoint, index: number) => 
    `${index === 0 ? 'M' : 'L'} ${point.x} ${point.y}`
  ).join(' ') + ' Z';

  const getValueColor = (value: number): ValueColor => {
    ...
    <div className="relative w-full max-w-2xl mx-auto">
      ...
      <div className="relative aspect-square w-full overflow-visible">
        ...
        >
          ...
          {[20, 40, 60, 80, 100].map((percent: number, index: number) => (
          ...
          {/* Axis lines */}
          {categories.map((_: ChartCategory, index: number) => {
            const angle: number = index * angleStep - Math.PI / 2;
            const x2: number = centerX + radius * Math.cos(angle);
            const y2: number = centerY + radius * Math.sin(angle);
          ...
          {/* Data points */}
          {points.map((point: RadarPoint, index: number) => {
            const colors: ValueColor = getValueColor(point.category.value);
          ...
          {/* Category labels */}
          {categories.map((cat: ChartCategory, index: number) => {
            const angle: number = index * angleStep - Math.PI / 2;
            const labelRadius: number = radius + 40;
            const x: number = centerX + labelRadius * Math.cos(angle);
            const y: number = centerY + labelRadius * Math.sin(angle);
            const colors: ValueColor = getValueColor(cat.value);
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-300 leading-relaxed">
                {(() => {
                  const avgRisk: number = categories.reduce((sum: number, cat: ChartCategory) => sum + cat.value, 0) / categories.length;
                  const highRiskCategories: ChartCategory[] = categories.filter((cat: ChartCategory) => cat.value > 66);
              ...
              <div className="mt-4">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-xs font-bold text-purple-400">
                    {Math.round(categories.reduce((sum: number, cat: ChartCategory) => sum + cat.value, 0) / categories.length)}%
                  </span>
                </div>
                <div className="h-2 bg-gray-700/50 rounded-full overflow-hidden">
                  <motion.div
                    ...
                    initial={{ width: 0 }}
                    animate={{ 
                      width: `${categories.reduce((sum: number, cat: ChartCategory) => sum + cat.value, 0) / categories.length}%` 
                    }}
