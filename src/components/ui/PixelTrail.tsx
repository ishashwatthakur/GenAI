...
// Fade duration stage interface
interface FadeStage {
  opacity: number;
  delay: number;
}

export default function PixelTrail(): JSX.Element {
  ...
  useEffect(() => {
    ...
    const pixelSize: number = 60; // 60px pixels as requested
    const cols: number = Math.ceil(window.innerWidth / pixelSize);
    const rows: number = Math.ceil(window.innerHeight / pixelSize);
    const totalPixels: number = cols * rows;
    ...
    for (let i = 0; i < totalPixels; i++) {
      ...
      pixel.addEventListener('mouseenter', function(this: HTMLDivElement) {
        ...
        trailRef.current.forEach((trailPixel: HTMLDivElement, index: number) => {
      ...
      pixel.addEventListener('mouseleave', function(this: HTMLDivElement) {
        const self: HTMLDivElement = this;
        let fadeStep: number = 0;
        
        // Gradual fade out stages
        const fadeDurations: FadeStage[] = [
        ...
        fadeInterval = setInterval(() => {
          if (fadeStep >= fadeDurations.length) {
            ...
            // Remove from trail
            const index: number = trailRef.current.indexOf(self);
            if (index > -1) {
          ...
          const stage: FadeStage = fadeDurations[fadeStep];
